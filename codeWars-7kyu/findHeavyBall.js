...
function findBall(scales) {
    let panes = [0, 1, 2, 3, 4, 5, 6, 7]; 
    for (let i = 0; panes.length > 1; i++) 
        panes =  scales.getWeight(panes.slice(0, panes.length/2), panes.slice(panes.length/2)) == -1 ?
                  panes.slice(0, panes.length/2) : panes.slice(panes.length/2);     
    return panes[0];
}